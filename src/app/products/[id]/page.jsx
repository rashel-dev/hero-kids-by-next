...
export async function generateMetadata({ params }) {
    ...
    return {
        ...
        openGraph: {
            title: product.title,
            description: "Fun and educational learning toy for kids. Safe, colorful, and engaging.",
            images: [
                {
    ...
    return (
        <div className={`  max-w-6xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-10`}>
            ...
            <div className="rounded-xl overflow-hidden ">
                <Image width={600} height={420} src={image} alt={title} className="w-full h-105 object-cover" />
            </div>
                ...
                <div className="flex items-center gap-2 mb-4">
                    <div className="flex text-yellow-400">
                        {Array.from({ length: 5 }, (_, i) => (
                            <FaStar key={i} className={i < Math.round(ratings) ? "" : "opacity-30"} />
                        ))}
                    </div>
