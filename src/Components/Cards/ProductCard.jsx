    ...
    return (
        <div className="card  bg-base-100 shadow-lg relative">
            <figure className="p-4">
                <Image width={200} height={180} src={image} alt={title} className="rounded-md w-full h-48 object-cover" />
            </figure>
                ...
                <div className="flex items-center space-x-2 mt-2">
                    <div className="flex text-yellow-400">
                        {Array.from({ length: 5 }, (_, i) => (
                            <FaStar key={i} className={i < Math.round(ratings) ? "opacity-100" : "opacity-40"} />
                        ))}
                    </div>
